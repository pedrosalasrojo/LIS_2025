...
# Store dataset names, as an example Luxembourg 2021 from LWS
ccyy <- c('de20')  
results <- NA 
...
# Print name of dataset 
 print(ccyy) 
  ...
  # Arrange the data, just fix age, get pilabour over 10€ and arrange to show USD2017 with PPP
  data <- data %>% 
    dplyr::filter(age>=30 & age<=60 & pilabour > 10) %>%
    dplyr::mutate(pilabour = pilabour/0.965991,
  ...
  lambda_try <- 225       # Set el lambda a 225, as an example
  ...
  # EXERCISE 2: Train LASSO to select opt* lambda
  # Define LASSO setting 
  
  dep <- data$pilabour
  vec <- model.matrix(~ sex + factor(marital) + factor(ind1_c) + 
                      factor(occ1_c) + disabled  + factor(status1) + 
                      factor(educlev) + factor(age5num), data)

  # Define optimum lambda grid
  ...
  # Get LASSO and get coefficients
